...
class TrainerBase(nn.Module):
    ...
    @staticmethod
    def save_best_model(model, epoch, loss):
        torch.save(model.state_dict(), f'checkpoints/Model_epoch{epoch}_loss{loss}.pth')
        print(f"成功将此次训练模型存储(储存格式为.pth)至:.'checkpoints/Model_epoch{epoch}_loss{loss}.pth'")
        ...
        with open("log.txt", "w") as f:
            for i in range(self.epoches):
                ...
                if "model_save_path" in kwargs.keys():
                    model_save_path = f'checkpoints/Model_epoch{i}_loss{loss}.pth'
                    torch.save(model.state_dict(), model_save_path)

                # 保存每个 step 的损失信息到 log.txt
                f.write(f"Epoch [{i}/{self.epoches}] Loss:{loss}\n")
